// 
// define width and height for plot area
var width = 500;
var height = 600;
var projection = d3.geoMercator()
    .scale(40000)
    // point to the center of LA
    .center([-118.245, 34.05])
    .translate([width / 2 + 100, height / 2]);

// Using D3 to select using toolTip
//
...
var color;
// Queue all geojson files
//
var queue = d3.queue();
queue.defer(d3.json, "./data-source/lapd-divisions.geojson")
queue.defer(d3.json, "./data-source/dataByAreaV2.json")
queue.defer(d3.json, "./data-source/dataOverall.json")
queue.defer(d3.json, "./data-source/timeOccurredTotal.json")
queue.defer(d3.json, "./data-source/timeOccurredTotalByYear.json")
queue.defer(d3.json, "./data-source/timeOccurredByArea.json")
queue.await(ready);
var select_years = ['All', '2017', '2016', '2015', '2014', '2013', '2012', '2011', '2010'];

// Define selection: year (All), area, data, etc.
//
...
dispatch.on("load.reset", function () {
    ...
    button.text("LAPD Divisional Areas ");
...
//  pie chart to display population by age group; 
...
dispatch.on("statechange.title", function (y) {
    d3.select("body")
        ...
        .text((selected_area == null ? "LAPD Divisional Areas" : selected_area)
        + " - " + (y == "All" ? "All Year Range" : y));
});
dispatch.on("areachange.title", function (data) {
    ...
    d3.select("body")
        .select("div#area")
        .select("span#current-area")
        .text((selected_area ? selected_area : "LAPD Divisional Areas") + " - " + (selected_year == "All" ? "All Year Range" : selected_year));
});
